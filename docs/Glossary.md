...
| Message        | 消息            | `messages/`                      | `chat/`              | `chat/`                            | 私信             | `/api/v1/chats/{chat}/messages` | 子资源；实时经 Socket.IO                         |
...
| Explore        | 探索            | `post/` + ExploreService         | `explore/`           | `explore/`                         | 探索网格         | `/api/v1/posts/explore`         | 内存缓存                                         |
| Comment        | 评论            | `comments/`                      | `feed/components/`   | `feed/` / `app/post-comments`      | 评论弹窗         | `/api/v1/posts/{post}/comments` | 子资源                                           |
| Follow         | 关注            | `follow/`                        | `profile/`           | `profile/`                         | 粉丝 / 关注      | `/api/v1/users/{user}:follow`   | AIP-136 custom method                            |
...
| AIP REST                        | AIP REST         | Google API Improvement Proposals 风格的 REST：资源路径、标准方法、page_token、RpcStatus 错误；前缀仍为 `/api/v1` |
