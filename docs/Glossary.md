...
| Feed           | 信息流          | GraphQL + feed services                  | 首页 Feed        | `/api/v1/graphql`            | Query `feed`；另有 REST `GET /api/v1/posts/feed` |
| Reels          | 短视频          | GraphQL                                  | Reels Tab        | `/api/v1/graphql`            | Query `reels`                                    |
| Explore        | 探索            | `presentation/post` + ExploreService     | 探索网格         | `/api/v1/posts/explore`      | Redis 缓存                                       |
...
| Status         | 状态            | `presentation/status`                    | Status UI        | `/api/v1/status`             | 24h 状态                                         |
| Image Gen      | 图片生成        | `presentation/image`                     | 生成对话框       | `/api/v1/image`              | Nest → media-gen                                 |
| Video Gen      | 视频生成        | `presentation/video`                     | 生成对话框       | `/api/v1/video`              | Nest → media-gen                                 |
| Voice Gen      | 语音合成        | `presentation/voice`                     | 生成对话框       | `/api/v1/voice`              | Nest → media-gen                                 |
...
| Explore AI BFF | Explore AI 代理 | `presentation/ai`（ExploreAiController） | 经 Nest 代理     | `/api/v1/ai/explore`         | 客户端勿直连 Explore AI                          |
