...
export class ExploreAiClientService {
  ...
  async streamChat(userId: string, body: ExploreAiStreamChatBody): Promise<Response> {
    this.assertAvailable();
  ...
  async listModels(userId: string, provider?: string): Promise<unknown> {
    ...
    return this.requestJson(userId, `/api/text/models${query}`, { method: "GET" });
  }

  async createSession(userId: string, body: ExploreAiCreateSessionBody): Promise<unknown> {
  ...
  async getSession(userId: string, sessionId: string): Promise<unknown> {
    return this.requestJson(userId, `/api/sessions/${encodeURIComponent(sessionId)}`, {
      method: "GET",
    });
  }

  async getMessages(userId: string, sessionId: string): Promise<unknown> {
    return this.requestJson(userId, `/api/sessions/${encodeURIComponent(sessionId)}/messages`, {
      method: "GET",
    });
  }

  async deleteSession(userId: string, sessionId: string): Promise<void> {
    await this.requestJson(userId, `/api/sessions/${encodeURIComponent(sessionId)}`, {
      method: "DELETE",
    });
  }
  ...
  private async requestJson(
    ...
    const timeout = setTimeout(() => controller.abort(), this.config.exploreAi.timeoutMs);
    try {
      const res = await fetch(url, {
        ...init,
        headers: {
